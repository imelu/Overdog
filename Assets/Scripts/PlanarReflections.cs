...
namespace WaterSystem.Rendering
{
    public class PlanarReflections
    {
        ...
        public class PlanarReflectionSettings
        {
            ...
            public int2 m_ResolutionCustom = new int2(1920, 1080);
        ...
        private static void UpdateReflectionCamera(Camera realCamera, Transform transform)
        {
            ...
            // !! CHANGED FROM ORIGINAL SCRIPT TO PRESERVE CULLING MASKS
            //_reflectionObjects[realCamera].Camera.cullingMask = m_settings.m_ReflectLayers; // never render water layer 
            InvertPlayerReflections(realCamera); // disable this to revert the inversed reflections
        ...
        private static RenderTexture CreateTexture(int2 res)
        {
            ...
            return RenderTexture.GetTemporary(1920, 1080, 24,
        ...
        // custom function to invert the reflections
        private static void InvertPlayerReflections(Camera realCamera)
        {
            _reflectionObjects[realCamera].Camera.cullingMask ^= 1 << LayerMask.NameToLayer("Player1");
            _reflectionObjects[realCamera].Camera.cullingMask ^= 1 << LayerMask.NameToLayer("Player2");
            _reflectionObjects[realCamera].Camera.cullingMask &= ~(1 << LayerMask.NameToLayer("Player1Floor"));
            _reflectionObjects[realCamera].Camera.cullingMask &= ~(1 << LayerMask.NameToLayer("Player2Floor"));
        }
